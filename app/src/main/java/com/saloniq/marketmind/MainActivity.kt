...
private fun MarketMindApp() {
    ...
    MarketMindTheme(darkTheme = darkMode) {
        ...
        when (screen) {
            "add" -> AddAssetScreen({ screen = "dashboard" }) { asset -> if (watchlist.none { it.symbol.equals(asset.symbol,true) }) { watchlist.add(asset); settings.saveAssets(watchlist); refreshKey++ }; screen="dashboard" }
...
@Composable private fun DashboardScreen(assets:List<Asset>, repository:MarketRepository, onAdd:()->Unit, onSettings:()->Unit, onRemove:(Asset)->Unit, onRefresh:()->Unit) {
    ...
    Column(Modifier.fillMaxSize()) { Row(Modifier.fillMaxWidth().padding(16.dp),Arrangement.SpaceBetween){ Column{Text("MarketMind",style=MaterialTheme.typography.headlineMedium);Text("Stocks + crypto • ${aiSettings.provider().label}")};OutlinedButton(onClick=onSettings){Text("Settings")} }
        LazyColumn(Modifier.fillMaxWidth().padding(horizontal=16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){ item{Button(onClick=onRefresh,modifier=Modifier.fillMaxWidth()){Text("Refresh market data")}};items(assets,key={it.symbol}){asset->AssetCard(asset,repository.cachedQuote(asset.symbol),{aiLoading=true;scope.launch{val q=withContext(Dispatchers.IO){repository.fetchQuote(asset)};aiText=withContext(Dispatchers.IO){AiClient(context).analyze(asset,q)};aiLoading=false}},{onRemove(asset)})};item{Button(onClick=onAdd,modifier=Modifier.fillMaxWidth()){Text("Add stock or crypto")};if(aiLoading)Text("AI is analyzing…",Modifier.padding(top=8.dp));if(aiText.isNotBlank())Card(Modifier.fillMaxWidth()){Text(aiText,Modifier.padding(16.dp))};Spacer(Modifier.height(24.dp))}}
    }
}

@Composable private fun AssetCard(asset:Asset,quote:Quote?,onAnalyze:()->Unit,onRemove:()->Unit){Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp)){Row(Modifier.fillMaxWidth(),Arrangement.SpaceBetween){Column{Text(asset.symbol,style=MaterialTheme.typography.titleLarge);Text(asset.name)};Text(asset.type,style=MaterialTheme.typography.labelMedium)};Spacer(Modifier.height(10.dp));Text("Price: ${quote?.price?.let{String.format("%.4f",it)}?:"—"}");Text("24h: ${quote?.change24h?.let{String.format("%+.2f%%",it)}?:"—"}");Text(quote?.signal?:"Waiting for market data",style=MaterialTheme.typography.bodySmall);Spacer(Modifier.height(10.dp));Row(Modifier.fillMaxWidth(),Arrangement.spacedBy(8.dp)){Button(onClick=onAnalyze,modifier=Modifier.weight(1f)){Text("AI analysis")};OutlinedButton(onClick=onRemove){Text("Remove")}}}}}

@Composable private fun AddAssetScreen(onBack:()->Unit,onAdd:(Asset)->Unit){var symbol by remember{mutableStateOf("")};var name by remember{mutableStateOf("")};var type by remember{mutableStateOf("Stock")};var marketSymbol by remember{mutableStateOf("")};var coinId by remember{mutableStateOf("")};Column(Modifier.fillMaxSize().padding(16.dp)){Text("Add asset",style=MaterialTheme.typography.headlineMedium);Text("Add stocks or crypto to your personal watchlist.",Modifier.padding(vertical=8.dp));OutlinedTextField(value=symbol,onValueChange={symbol=it.uppercase()},label={Text("Ticker / symbol")},modifier=Modifier.fillMaxWidth());OutlinedTextField(value=name,onValueChange={name=it},label={Text("Name")},modifier=Modifier.fillMaxWidth());Spacer(Modifier.height(8.dp));Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){Button(onClick={type="Stock"}){Text("Stock")};OutlinedButton(onClick={type="Crypto"}){Text("Crypto")}};if(type=="Stock")OutlinedTextField(value=marketSymbol,onValueChange={marketSymbol=it},label={Text("Market ticker, e.g. TSLA or CDR.WA")},modifier=Modifier.fillMaxWidth()) else OutlinedTextField(value=coinId,onValueChange={coinId=it.lowercase()},label={Text("CoinGecko ID, e.g. bitcoin")},modifier=Modifier.fillMaxWidth());Spacer(Modifier.height(16.dp));Button(onClick={if(symbol.isNotBlank())onAdd(Asset(symbol,name.ifBlank{symbol},type,marketSymbol.ifBlank{symbol},coinId.ifBlank{null}))},enabled=symbol.isNotBlank(),modifier=Modifier.fillMaxWidth()){Text("Add to watchlist")};OutlinedButton(onClick=onBack,modifier=Modifier.fillMaxWidth()){Text("Back")}}
